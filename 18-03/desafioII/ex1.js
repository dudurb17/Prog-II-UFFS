...
const n = Number(prompt("Informe o número N de medidas:").trim());
const alturas = prompt("Informe as N alturas:").trim().split(/\s+/).map(Number);

let resultado = 0;
if (n > 1 && alturas.length === n) {
  let anterior = Math.sign(alturas[1] - alturas[0]);
  if (anterior !== 0) {
    resultado = 1;
    for (let i = 2; i < n; i++) {
      const atual = Math.sign(alturas[i] - alturas[i - 1]);
